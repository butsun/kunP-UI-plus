  ...
  <div class="p-2">
      ...
      <div v-show="showSearch" class="mb-[10px]">
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            ...
            <el-form-item label="归属公司ID" prop="enterpriseId">
              <el-input v-model="queryParams.enterpriseId" placeholder="请输入归属公司ID" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            ...
            <el-form-item label="是否禁用凭证:0、启用;1、禁用" prop="disableFlag">
              <el-input v-model="queryParams.disableFlag" placeholder="请输入是否禁用凭证:0、启用;1、禁用" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            <el-form-item>
    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['kp-system:chargeVoucher:add']">新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['kp-system:chargeVoucher:edit']">修改</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['kp-system:chargeVoucher:remove']">删除</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['kp-system:chargeVoucher:export']">导出</el-button>
          </el-col>
      ...
      <el-table v-loading="loading" :data="chargeVoucherList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="归属公司ID" align="center" prop="enterpriseId" />
        <el-table-column label="凭证类型:0 手机号; 1、卡; 2、 VIN;" align="center" prop="voucherType" />
        <el-table-column label="归属账户ID" align="center" prop="accountId" />
        <el-table-column label="是否禁用凭证:0、启用;1、禁用" align="center" prop="disableFlag" />
        <el-table-column label="备注" align="center" prop="remark" />
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['kp-system:chargeVoucher:edit']"></el-button>
            </el-tooltip>
            <el-tooltip content="删除" placement="top">
              <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['kp-system:chargeVoucher:remove']"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog :title="dialog.title" v-model="dialog.visible" width="500px" append-to-body>
      <el-form ref="chargeVoucherFormRef" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="归属公司ID" prop="enterpriseId">
          <el-input v-model="form.enterpriseId" placeholder="请输入归属公司ID" />
        </el-form-item>
        <el-form-item label="归属账户ID" prop="accountId">
          <el-input v-model="form.accountId" placeholder="请输入归属账户ID" />
        </el-form-item>
        <el-form-item label="是否禁用凭证:0、启用;1、禁用" prop="disableFlag">
          <el-input v-model="form.disableFlag" placeholder="请输入是否禁用凭证:0、启用;1、禁用" />
        </el-form-item>
...
import { listChargeVoucher, getChargeVoucher, delChargeVoucher, addChargeVoucher, updateChargeVoucher } from '@/api/kp-system/chargeVoucher';
import { ChargeVoucherVO, ChargeVoucherQuery, ChargeVoucherForm } from '@/api/kp-system/chargeVoucher/types';
...
const data = reactive<PageData<ChargeVoucherForm, ChargeVoucherQuery>>({
  ...
  rules: {
    ...
    enterpriseId: [
      { required: true, message: "归属公司ID不能为空", trigger: "blur" }
    ],
    voucherType: [
      { required: true, message: "凭证类型:0 手机号; 1、卡; 2、 VIN;不能为空", trigger: "change" }
    ],
    accountId: [
...
/** 导出按钮操作 */
const handleExport = () => {
  proxy?.download('kp-system/chargeVoucher/export', {
    ...queryParams.value
